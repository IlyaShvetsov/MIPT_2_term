...
struct Vertice
{
	int color;
	int len;
	Vertice() : color(0), len(0) {}
};

bool bfs(const IGraph& Graph, int vertice, int& )
{
	queue<int> q;
...
int Find(const IGraph& Graph)
{
	...
	for (int i = 0; i < Graph.VerticesCount() && min != 3; ++i)	// если нашли цикл размера 3, выходим
	{
		result = bfs(Graph, i);
		if (result < min)
			min = result;
	...
	if (min == 1000000)
		return -1;											// если не нашли цикл, возвращаем минус 1
	else
