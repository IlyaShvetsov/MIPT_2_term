...
struct Vertex {
    ...
    bool is_right;
    bool is_left;
    Vertex() : isChecked(false), is_right(false), is_left(false) {}
};

void FindDistances(CAdjacencyMatrix& Graph, vector<Vertex>& vertices) {
...
bool dfs(const CAdjacencyMatrix& Graph, vector<Vertex>& vertices, int index, double table, int radius) {
...
double BinSearch(const CAdjacencyMatrix& Graph, vector<Vertex> vertices, int xr, int radius) {
    double table = xr;
    double max = table;
    double min = 0;
