...
int Dijkstra(const CListGraph& Graph, int vert_1, int vert_2)
{
	...
	while (!isChecked[vert_2] && !q.empty())
	{
		vertice = *q.begin();
		q.erase(q.begin());
		isChecked[vertice.second] = true;
		vector <pair <int, int> > vertices;
		Graph.GetNextVertices(vertice.second, vertices);
		for (int j = 0; j < vertices.size(); ++j)
			if (!isChecked[vertices[j].second] && vertice.first + vertices[j].first < dist[vertices[j].second])
			{
				if (dist[vertices[j].second] != INT_MAX)
					q.erase(make_pair(dist[vertices[j].second], vertices[j].second));
				q.insert(pair <int, int>(vertice.first + vertices[j].first, vertices[j].second));
				dist[vertices[j].second] = vertice.first + vertices[j].first;
			}
	}
	return vertice.first;
}
